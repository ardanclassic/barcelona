...
export function showAddBookmarkConfirm(btn, name) {
	M.Toast.dismissAll();
	btn.style.backgroundColor = "#B71C1C";
	btn.innerText = "Delete from Favorite?";
	M.toast({
		inDuration: 100,
		html: `<span>Save <strong class="yellow-text">${name}</strong> as your favorite!</span>`
	})
}

export function showRemoveBookmarkConfirm(btn, name) {
	M.Toast.dismissAll();
	btn.style.backgroundColor = "#114875";
	btn.innerText = "Save as Your Favorite!";
	M.toast({
		inDuration: 100,
		html: `<span>Remove <strong class="yellow-text">${name}</strong> from your favorite!</span>`
	})
}
